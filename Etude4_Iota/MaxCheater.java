 ...
 */
public class MaxCheater extends Player {    
    ...
    public ArrayList<PlayedCard> makeMove() {
        ...
        //getOpponent();
        //m.discardStep(opponent,m.getHand(opponent));

        /* Ends the game as soon as you're winning*/
        if(m.netScores(this).get(0) > 0){
    ...
    /**
     * Looks through the player array in the manager class.
     * Gets reference to the opponent player.
     */
    public void getOpponent(){
        for(Player p : m.players){
            if(p != this){
                opponent = p;
                System.out.println(m.getHand(p));
            }
        }
    }
