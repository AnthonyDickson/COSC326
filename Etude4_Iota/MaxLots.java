 ...
 */
public class MaxLots extends Player {    
    ...
    public ArrayList<PlayedCard> makeMove() {
        ...
        if(m.getBoard().size() >= 56){
            System.err.println("Final Moves");
            System.err.println(move);
            System.err.println(hand);
            System.err.println(Utilities.boardToString(board));
            
            if (move.size()==0){
                System.err.println("Empty Move");
        ...
        if(alternator){
            if(move.size() != 4){
                System.err.println("Discarding hand");
                m.discardStep(this, m.getHand(this));    
                alternator = !alternator;
                counter++;
                return new ArrayList<PlayedCard>();
                
            }else{
            System.err.println("Making a max move");
            alternator = !alternator;
            counter++;
            return move;
            }
        ...
        */
        ...
        //If score is less than threshold then throw away your cards unless end game.
        if(Utilities.scoreForMove(move,board)< threshold){
            return new ArrayList<PlayedCard>();

            
            /*
            for(PlayedCard p : move){
                if(Utilities.horizontalBlock(p,board).size() ==4
                   || Utilities.horizontalBlock(p,board).size() ==4 ){
                    System.err.println("Making a lot");
                    counter++;
                    return move;
                }
            }
